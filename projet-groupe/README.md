...
### ❌ Problème : Kafka - NoBrokersAvailable
...
### ❌ Problème : MapReduce - Job Failed

**Symptôme :**
```
Application application_XXX_YYY failed 2 times
```
...
# Relancer le job
docker exec resourcemanager hadoop jar /opt/hadoop-3.2.1/share/hadoop/mapreduce/hadoop-mapreduce-examples-3.2.1.jar wordcount /data /output2
```

### ❌ Problème : Dashboard - Port 8504 occupé
...
**Symptôme :**
```
Container namenode exited with code 1
```
...
**Principe :**
- Stockage distribué sur plusieurs nœuds (DataNodes)
- NameNode gère les métadonnées
- Réplication des données pour la tolérance aux pannes

**Dans notre projet :**
- 1 NameNode + 1 DataNode
- 25 MB de données (4 fichiers)
- Réplication factor = 1

### MapReduce - Traitement Batch

**Principe :**
- **Map** : Transformation des données en paires clé-valeur
- **Shuffle** : Regroupement par clé
- **Reduce** : Agrégation des valeurs

**Dans notre projet :**
- Job WordCount en Java
- Map : (mot, 1)
- Reduce : somme des occurrences
- Résultat : 33,544 mots uniques

### Kafka - Streaming Temps Réel

**Principe :**
- **Producer** : Envoie des messages vers un topic
- **Topic** : File d'attente distribuée
- **Consumer** : Lit les messages du topic

**Dans notre projet :**
- Producer Python : hdfs_to_kafka.py
- Topic : bank-transactions
- Consumer Python : ecommerce_analyzer.py
- 127,557+ transactions streamées

### Architecture Lambda

**Principe :**
- **Batch Layer** : MapReduce pour analyse historique
- **Speed Layer** : Kafka pour traitement temps réel
- **Serving Layer** : Dashboard pour visualisation

**Dans notre projet :**
- HDFS → MapReduce (batch)
- HDFS → Kafka (streaming)
- Dashboard Streamlit (serving)
...
- **Hadoop** : https://hadoop.apache.org/docs/r3.2.1/
...
|-------------|---------|------|
| Docker Compose | 3.8 | Orchestration conteneurs |
| Apache Hadoop | 3.2.1 | HDFS + MapReduce |
| Apache Kafka | 7.5.0 | Message Broker |
| Zookeeper | 7.5.0 | Coordination Kafka |
| Python | 3.11+ | Scripts ETL & Analytics |
| Streamlit | 1.40+ | Dashboard interactif |
| Plotly | 5.24+ | Visualisation graphiques |
| kafka-python-ng | 2.2+ | Client Kafka Python |

### Scripts Python Clés

```python
# kafka/hdfs_to_kafka.py (127,557 events)
...
# kafka/ecommerce_analyzer.py (2,209 patterns)
...
✅ **Infrastructure Big Data opérationnelle** (7 services Docker)  
✅ **25 MB de données stockées** dans HDFS (objectif : 20 MB)  
✅ **111,197 mots uniques analysés** avec MapReduce (objectif : 100K)  
✅ **127,557 transactions streamées** via Kafka (objectif : 100K)  
✅ **2,209 patterns détectés** en temps réel (objectif : 2K)  
✅ **Dashboard 4 pages** avec visualisations interactives  
✅ **Taux de conversion 65%** (objectif : 50%)  
✅ **Documentation complète** (README, RAPPORT, SPEECH)  

### Points Forts

1. **Architecture hybride** : Batch (MapReduce) + Streaming (Kafka)
2. **Automatisation complète** : Script lancer_tout.ps1
3. **Visualisation temps réel** : Dashboard Streamlit
4. **Performance** : 17 secondes pour 2.8M mots, 500 events/s Kafka
5. **Cas d'usage concret** : E-commerce analytics avec ROI calculé

### Améliorations Possibles

1. **Scalabilité** : Ajouter 2+ DataNodes, 3+ Kafka brokers
2. **Machine Learning** : Prédiction d'abandon de panier
3. **Monitoring** : Prometheus + Grafana
4. **CI/CD** : Tests automatisés, deployment pipeline
5. **Sécurité** : Kerberos, SSL/TLS
...
**Date** : Décembre 2025  
**Environnement** : Windows PowerShell, Docker Desktop  

---

## � Licence

Projet éducatif - École d'ingénieur Big Data  
...
docker exec namenode hdfs dfs -ls /data              # 25 MB
docker exec namenode hdfs dfs -cat /output/part-r-00000 | head -n 20  # 111,197 mots
docker exec broker kafka-topics --list --bootstrap-server localhost:9092  # bank-transactions
```

**🎉 Projet opérationnel en 5 minutes !**

---

**Développé avec ❤️ pour l'apprentissage du Big Data**
